...
class Set:
    ...
    def __getitem__(self, index):
        ...
        """
        Returns the object at paramater index

        process: receives and gives back the item at the stated index, starting at 0.
        precondition(s): index is an integer, that exists at the number of objects in the
        set minus one.
        postcondition(s): the object at the given index is given back to the programmer.
        """
    ...
    def insert(self, index, item):
        """
        Inserts paramater item at paramater index within the set.

        process: Takes an item, and mutates the set so that item is in the correct index.
        precondition(s): A proper index exists for which the item to be placed into.
        postcondition(s): The set is changed to include the item at the given index.
        """
        if item not in self._contents:
            self._contents.insert(index, item)
        else:
            raise RuntimeError("Item already in Set.")
...
if __name__=='__main__':
    ...
    print "Tom.insert(2, 'water')"
    Tom.insert(2, 'water')
    print "print Tom"
    print Tom
    print "Try/else:"
    print "Tom.insert(3, 'water')"
    try:
        print Tom.insert(3, 'water')
    except:
        print "RuntimeError!"
